...
(() => {
	...
	var L = "kit",
		...
		U = "Kit",
		J = "Kit Duotone",
		_$t = _defineProperty(_defineProperty({}, L, U), r, J);
	var ol = {
		kit: {
			"fa-kit": "fak",
		},
	...
	var E$1 = "kit",
		...
		_$1 = "Kit",
		M$1 = "Kit Duotone",
		_rt$1 = _defineProperty(_defineProperty({}, E$1, _$1), d$1, M$1);
	var $t$1 = {
			classic: {
				"fa-brands": "fab",
				"fa-duotone": "fad",
		...
		to = []
			...
			.concat(O$1.map((t) => "".concat(t, "x")))
			.concat(V$1.map((t) => "w-".concat(t)));
	var ro = {
		"Font Awesome 5 Free": {
			900: "fas",
			400: "far",
	...
	var build = function build() {
		...
		_byUnicode = lookup((acc, icon, iconName) => {
			...
			if (icon[2]) {
				var aliases = icon[2].filter((a$$1) => typeof a$$1 === "number");
		...
		_byLigature = lookup((acc, icon, iconName) => {
			...
			if (icon[2]) {
				var aliases = icon[2].filter((a$$1) => typeof a$$1 === "string");
				aliases.forEach((alias) => {
					acc[alias] = iconName;
				});
			}
	...
	function getFamilyId(values) {
		var family = i;
		var famProps = FAMILY_NAMES.reduce((acc, familyId) => {
	...
	function getCanonicalIcon(values) {
		...
		var faStyleOrFamilyClasses = sortedUniqueValues(
			values.filter((cls) => _faCombinedClasses.includes(cls)),
		);
		var nonStyleOrFamilyClasses = sortedUniqueValues(
			values.filter((cls) => !_faCombinedClasses.includes(cls)),
		);
		var faStyles = faStyleOrFamilyClasses.filter((cls) => {
			givenPrefix = cls;
	...
	var newCanonicalFamilies = rt.filter(
		(familyId) => familyId !== i || familyId !== t,
	);
	var newCanonicalStyles = Object.keys(Ht$1)
		.filter((key) => key !== i)
		.flatMap((key) => Object.keys(Ht$1[key]));
	function getDefaultCanonicalPrefix(prefixOptions) {
		...
		if (!canonical.prefix && newCanonicalFamilies.includes(family)) {
			var validPrefix = Object.keys(styles).find((key) =>
	...
	function isLabeled(attributes) {
		...
		return labels.some((label) => label in attributes);
	}
	...
	function hasBeenReplaced(node) {
		return node?.classList?.contains?.(config.replacementClass);
	}
	function getMutator() {
	...
	var mutators = {
		...
		nest: function nest(mutation) {
			...
			var newInnerHTML = abstract.map((a) => toHtml(a)).join("\n");
	...
	function attributesParser(node) {
		var extraAttributes = toArray(node.attributes).reduce((acc, attr) => {
	...
	function onTree(root) {
		...
		var mutations = candidates.reduce((acc, node) => {
			try {
		...
		provides: function provides(providers$$1) {
			providers$$1.i2svg = (params) => {
				var _params$node = params.node,
					...
					callback = _params$callback === void 0 ? () => {} : _params$callback;
			...
			providers$$1.generateAbstractIcon = (_ref3) => {
	...
	var LayersText = {
		...
		provides: function provides(providers$$1) {
			providers$$1.generateLayersText = (node, nodeMeta) => {
				...
				var height = null;
				if (IS_IE) {
					var computedFontSize = Number.parseInt(
	...
	function getPrefix(fontFamily, fontWeight) {
		var fontFamilySanitized = fontFamily
			.replace(/^['"]|['"]$/g, "")
			.toLowerCase();
		var fontWeightInteger = Number.parseInt(fontWeight, 10);
		var fontWeightSanitized = Number.isNaN(fontWeightInteger)
			? "normal"
			: fontWeightInteger;
		return (
	...
	function replaceForPosition(node, position) {
		...
		return new Promise((resolve, reject) => {
			...
			var alreadyProcessedPseudoElement = children.filter(
	...
	var hasPseudoElement = function hasPseudoElement(selector) {
		return (
			!!selector &&
			PSEUDO_ELEMENTS.some((pseudoSelector) =>
				selector.includes(pseudoSelector),
			)
		);
	};
	...
	var parseCSSRuleForPseudos = function parseCSSRuleForPseudos(selectorText) {
		...
		selectors = selectors.flatMap((selector) =>
			selector.includes("(")
				? selector
				: selector.split(",").map((s$$1) => s$$1.trim()),
		);
		...
		try {
			for (_iterator.s(); !(_step = _iterator.n()).done; ) {
				...
				if (hasPseudoElement(selector)) {
					...
					var selectorWithoutPseudo = PSEUDO_ELEMENTS.reduce(
						(acc, pseudoSelector) => acc.replace(pseudoSelector, ""),
						selector,
					);
