 ...
 *
 * Called after homelab-services-render.js populates cards (see window.initTruenasPageHealth).
 */
window.initTruenasPageHealth = function initTruenasPageHealth() {
	...
	function setIndicator(el, state, url) {
		...
		if (state === "pending") {
			i.className =
				"fas fa-circle-notch fa-spin truenas-health-indicator__icon truenas-health-indicator__icon--pending";
			el.setAttribute("aria-label", "Checking tunnel reachability");
			el.setAttribute(
				"title",
				"Checking whether the tunnel responds from your browser…",
			);
		} else if (state === "up") {
			i.className =
				"fas fa-circle truenas-health-indicator__icon truenas-health-indicator__icon--up";
			el.setAttribute("aria-label", "Tunnel appears reachable");
			el.setAttribute("title", "Tunnel responded (browser check): " + url);
		} else if (state === "down") {
			i.className =
				"fas fa-circle truenas-health-indicator__icon truenas-health-indicator__icon--down";
			...
			i.className =
				"fas fa-minus truenas-health-indicator__icon truenas-health-indicator__icon--na";
