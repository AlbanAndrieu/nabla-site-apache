...
test.describe('Responsive Design Tests', () => {
  ...
  test('should have touch-friendly interactive elements on mobile', async ({ page }) => {
    ...
    // Check that interactive elements are reasonably sized for touch
    for (let i = 0; i < Math.min(count, 10); i++) { // Check first 10 elements
      ...
      if (await element.isVisible()) {
        ...
        if (box) {
          ...
          // We'll be lenient and check for at least 40px in one dimension
          const hasReasonableSize = box.width >= 40 || box.height >= 40;
          expect(hasReasonableSize).toBeTruthy();
        }
      }
    }
  });
